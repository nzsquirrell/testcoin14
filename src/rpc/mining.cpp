...
UniValue generateBlocks(boost::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    {
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params()).CreateNewBlock(coinbaseScript->reserveScript, miningAlgo, NULL));
...
/*    if (IsInitialBlockDownload())
        ...
        fLastTemplateSupportsSegwit != fSupportsSegwit)
    {
        ...
        pblocktemplate = BlockAssembler(Params()).CreateNewBlock(scriptDummy, miningAlgo, NULL, fSupportsSegwit);
        if (!pblocktemplate)
...
/*    if (IsInitialBlockDownload() && !Params().MineBlocksOnDemand())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                           "Testcoin is downloading blocks...");*/
    ...
    if (request.params.size() == 0)
    {
                ...
                && GetTime() - nStart > 60))
        {
            ...
            std::unique_ptr<CBlockTemplate> newBlock(BlockAssembler(Params()).CreateNewBlock(coinbaseScript->reserveScript, miningAlgo, NULL));
            if (!newBlock)
                throw JSONRPCError(RPC_OUT_OF_MEMORY, "out of memory");
